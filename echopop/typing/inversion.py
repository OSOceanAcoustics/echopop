...
from pydantic import ConfigDict, Field, model_validator, RootModel, ValidationError
...
class InvParameters:
    ...
    def unscale(self):
        ...
        unscaled_params = {
            key: {
                **kwargs,
                "value": (
                    kwargs["value"] * (self.parameter_bounds[key]["max"] - 
    ...
    def unscale_dict(self, scaled_dict: dict) -> dict:
        ...
        return {
            k: scaled_dict[k] * 
            (self.parameter_bounds[k]["max"] - self.parameter_bounds[k]["min"]) + 
    ...
    def from_series(cls, series: pd.Series):
        ...
        if not isinstance(series, pd.Series):
            raise TypeError(
                f"InvParameters expects a `Series` for `.from_series`. Got type "
                f"{type(series)}."
            )
...
class MCInvParameters(InvParameters):
    ...
    def __str__(self):
        ...
        return f"MCInvParameters with {len(self.samples)} samples:\n" + \
            "\n".join([f"Sample {i}: {sample.values}" for i, sample in self.samples.items()])
