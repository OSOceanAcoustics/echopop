...
def test_fit_length_weight_regression_preserves_input():
    ...
    data = pd.DataFrame({
        "length": [10.0, np.nan, 20.0],
        "weight": [5.0, 15.0, 35.0],
        "other_col": ["A", "B", "C"]
    })
    data_copy = data.copy()
...
def test_fit_length_weight_regression_deterministic():
    ...
    data = pd.DataFrame({
        "length": [10.0, 15.0, 20.0, 25.0],
        "weight": [5.0, 15.0, 35.0, 70.0]
    })
...
def test_length_binned_weights_grouped_coefficients(
    ...
    result = biology.length_binned_weights(
        sample_specimen_data,
        sample_length_distribution,
        grouped_regression_coefficients
    )
...
def test_length_binned_weights_multiple_groups(
    ...
    """Test with multiple grouping variables."""
    result = biology.length_binned_weights(
        specimen_data_multiple_groups,
        sample_length_distribution,
        coefficients_with_multiple_groups
    )
...
def test_length_binned_weights_coefficient_types(
...
def test_length_binned_weights_realistic_coefficients(sample_specimen_data, sample_length_distribution):
