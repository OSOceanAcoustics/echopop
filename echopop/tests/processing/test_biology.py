...
def test_set_population_metrics_with_stratum_series(nasc_data_simple, average_weight_series):
    """
    Test the actual usage pattern: stratify_by="stratum_ks" with Series weights.
    """
    df = nasc_data_simple.copy()

    biology.set_population_metrics(
        df_nasc=df,
        metrics=["abundance", "biomass", "biomass_density"],
        stratify_by="stratum_ks",
        df_average_weight=average_weight_series,
    )

    # Check all columns were added
    assert "abundance" in df.columns
    assert "biomass" in df.columns
    assert "biomass_density" in df.columns

    # Check calculations for each row
    for idx, row in df.iterrows():
        stratum = row["stratum_ks"]
        weight = average_weight_series.loc[stratum]

        expected_abundance = round(row["area_interval"] * row["number_density"])
        expected_biomass = expected_abundance * weight
        expected_biomass_density = row["number_density"] * weight

        assert df.loc[idx, "abundance"] == expected_abundance
        assert abs(df.loc[idx, "biomass"] - expected_biomass) < 1e-10
        assert abs(df.loc[idx, "biomass_density"] - expected_biomass_density) < 1e-10


def test_set_population_metrics_abundance_only(nasc_data_simple):
    """
    Test calculating abundance only (no weights needed).
    """
    df = nasc_data_simple.copy()

    biology.set_population_metrics(df_nasc=df, metrics=["abundance"])

    assert "abundance" in df.columns
    assert "biomass" not in df.columns
    assert "biomass_density" not in df.columns

    # Check abundance calculations
    expected_abundance = np.round(df["area_interval"] * df["number_density"])
    pd.testing.assert_series_equal(df["abundance"], expected_abundance, check_names=False)


def test_set_population_metrics_scalar_weight(nasc_data_simple, scalar_weight):
    """
    Test with scalar weight (simpler case).
    """
    df = nasc_data_simple.copy()
    ...
    biology.set_population_metrics(
        df_nasc=df,
        metrics=["abundance", "biomass", "biomass_density"],
        df_average_weight=scalar_weight,
    )

    # Check calculations
    expected_abundance = np.round(df["area_interval"] * df["number_density"])
    expected_biomass = expected_abundance * scalar_weight
    expected_biomass_density = df["number_density"] * scalar_weight

    pd.testing.assert_series_equal(df["abundance"], expected_abundance, check_names=False)
    pd.testing.assert_series_equal(df["biomass"], expected_biomass, check_names=False)
    pd.testing.assert_series_equal(
        df["biomass_density"], expected_biomass_density, check_names=False
    )


def test_set_population_metrics_modifies_in_place(nasc_data_simple, scalar_weight):
    """
    ...
    result = biology.set_population_metrics(
        df_nasc=df, metrics=["abundance"], df_average_weight=scalar_weight
    )

    assert result is None  # Function returns None
    assert id(df) == original_id  # Same object
    assert "abundance" in df.columns  # Modified in place


def test_set_population_metrics_missing_stratum_column():
    """
    Test error when stratify_by column doesn't exist.
    """
    df = pd.DataFrame(
        {
    ...
    weight_series = pd.Series([0.4], index=[1])

    # Should not raise error if stratify_by column is missing - function should handle gracefully
    biology.set_population_metrics(
        df_nasc=df, metrics=["abundance"], stratify_by="stratum_ks", df_average_weight=weight_series
    )
