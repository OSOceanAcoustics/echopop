...
def test_JollyHampton_partition_data_into_transects(jolly_hampton, sample_ci_grid_data):
    ...
    data_with_transects, lat_key = jolly_hampton._partition_data_into_transects(
        sample_ci_grid_data
    )
...
def test_JollyHampton_initialize_virtual_df(jolly_hampton, sample_ci_grid_data):
    ...
    data_with_transects, _ = jolly_hampton._partition_data_into_transects(
        sample_ci_grid_data
    )
    
    virtual_df = jolly_hampton._initialize_virtual_df(
        data_with_transects, 'biomass'
    )
...
def test_JollyHampton_summarize_strata(sample_survey_ci_data):
    ...
    assert (jh.strata_summary['num_transects_to_sample'] <= 
            jh.strata_summary['transect_counts']).all()
...
def test_JollyHampton_compute_variance_static_method():
    ...
    # Compute means
    mean_values = (
        awk.sum(sampled_values * sampled_distances, axis=-1) / awk.sum(sampled_distances, axis=-1)
    )
...
def test_JollyHampton_extreme_sampling_proportion():
    ...
    jh_low.strata_summary = pd.DataFrame({
        'transect_counts': [10, 20],
        'area': [100, 200],
        'biomass': [1000, 2000]
    }, index=['stratum1', 'stratum2'])
    
    # Should ensure at least 1 transect per stratum
    sample_counts = np.round(
        jh_low.strata_summary['transect_counts'] * 0.01
    ).astype(int)
