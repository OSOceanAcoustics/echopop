...
def test_compute_binned_counts_multiple_filters(sample_specimen_data):
    ...
    expected_count = len(sample_specimen_data[
        (sample_specimen_data["sex"] != "unsexed") & 
        (sample_specimen_data["stratum_num"] != 1)
    ])
    assert result["count"].sum() == expected_count
...
def test_number_proportions_column_aliases(aged_dataframe, unaged_dataframe):
    ...
    result = get_proportions.number_proportions(
        aged_dataframe,
        unaged_dataframe,
        column_aliases=['specimen', 'length']
    )
...
def test_number_proportions_with_exclusion(aged_dataframe):
    ...
    stratum1_total = aged_dataframe[
        (aged_dataframe['stratum_num'] == 1) &
        (aged_dataframe["sex"] != "unsexed")
    ]['count'].sum()
    
    stratum1_female = aged_dataframe[
        (aged_dataframe['stratum_num'] == 1) & 
        (aged_dataframe['sex'] == 'female')
    ]['count'].sum()
    
    expected_proportion = stratum1_female / stratum1_total
    actual_proportion = result[
...
def test_number_proportions_proportion_calculations(aged_dataframe, unaged_dataframe):
    ...
    aged_df = result['aged']
    aged_female_row = aged_df[
        (aged_df['stratum_num'] == 1) & 
        (aged_df['sex'] == 'female')
    ].iloc[0]
...
def test_apply_weight_interpolation_by_contrast(length_dataset_with_bins, real_interpolators):
    ...
    sex_interp_dict = {
        'male': real_interpolators['male'],
        'female': real_interpolators['female']
    }
...
def test_apply_weight_interpolation_missing_interpolator(length_dataset_with_bins):
    ...
    male_interp = interp.interp1d(
        x_male, y_male, kind='linear', bounds_error=False,
        fill_value=(y_male[0], y_male[-1])
    )
...
def test_binned_weights_with_interpolation(length_dataset_with_bins, length_weight_dataset_with_bins):
...
def test_binned_weights_raises_error_without_dataset(length_dataset_with_bins):
    ...
    with pytest.raises(ValueError, match="length_weight_dataset must be provided when interpolate=True"):
...
def test_binned_weights_global_interpolation(length_dataset_with_bins, length_weight_dataset_with_bins):
...
def test_binned_weights_integration(length_dataset_with_bins, length_weight_dataset_with_bins):
    ...
    assert set(result_interp.columns.names) == set(result_no_interp.columns.names) == {'sex', 'stratum'}
