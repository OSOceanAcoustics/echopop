...
def test_partition_transect_data_abundance_only(sample_transect_dataset, age1_abundance_proportions):
    ...
    assert np.allclose(
        ...
        pd.DataFrame({
            "abundance": [440., 660., 820., 492.0, 702.0],
            "abundance_female": [220., 330., 410., 246., 351.],
            "abundance_male": [220., 330., 410., 246., 351.]
        })      
    )
...
def test_partition_transect_data_biomass_only(sample_transect_dataset, age1_biomass_proportions):
    ...
    result = apportion.partition_transect_data(
        dataset=sample_transect_dataset,
        partition_dict={"biomass": age1_biomass_proportions}
    )
    ...
    assert np.allclose(
        ...
        pd.DataFrame({
            "biomass": [230., 345., 430., 258., 369.],
            "biomass_female": [115., 172.5, 215., 129., 184.5],
            "biomass_male": [115., 172.5, 215., 129., 184.5]
        })      
    )

def test_partition_transect_data_all_variables(sample_transect_dataset, age1_nasc_proportions, 
                                             age1_abundance_proportions, age1_biomass_proportions):
    ...
    assert all([col in result.columns for col in [
        "nasc", "abundance", "abundance_female", "abundance_male", "number_density", "biomass", 
        "biomass_male", "biomass_female", "biomass_density"]
    ])
...
def test_set_biomass_with_weights(biology_test_dataset, average_weight_df):
    ...
    assert all([col in test_data.columns for col in [
        "biomass", "biomass_density", "biomass_density_female", "biomass_density_male"]
    ])
