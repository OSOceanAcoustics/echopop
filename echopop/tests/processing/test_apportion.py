...
def test_mesh_biomass_to_nasc(apportion_mesh, apportion_weight_proportions, 
                              apportion_stratum_weights, apportion_stratum_sigma_bs):
    """
    ...
    # Check columns
    assert (
        sorted(mesh_data_df.columns) == sorted(
            ["mesh_stratum", "biomass_density", "area", "biomass", "biomass_A",
             "biomass_B", "abundance", "abundance_A", "abundance_B", "nasc"]
        )
    )
...
def test_distribute_kriged_estimates(apportion_mesh_with_nasc, 
                                     apportion_weight_proportions,
                                     apportion_number_proportions):
    """
    ...
    biomass_tables = apportion.distribute_kriged_estimates(
        ...
        mesh_proportions_link={"mesh_stratum": "bio_stratum"},
    )
...
def test_standardize_kriged_estimates(apportion_biomass_table):
    ...
    assert all(std_tbl_no_imp.columns == pd.MultiIndex.from_tuples(
        [(1, "A"), (2, "A"), (1, "B"), (2, "B")]
        )
    )
    ...
    apportion_biomass_table_imp["subgroup1"].loc[[("A", 5, 1), ("B", 15, 2)]] = 0.
    # ---- Run standardization    
    ...
    assert all(std_tbl_imp.columns == pd.MultiIndex.from_tuples(
        [(1, "A"), (2, "A"), (1, "B"), (2, "B")]
        )
    )
...
def test_combine_population_tables(apportion_biomass_table_with_standardized):
    ...
    assert all(df_biomass_table.columns == pd.MultiIndex.from_tuples(
        [(1, "A"), (1, "B"), (2, "A"), (2, "B")]
        )
    )
    ...
    assert np.allclose(df_biomass_table - DF_SUM, 0.)
...
def test_redistribute_population_table(apportion_combined_biomass_table):
    ...
    # Check total sum
    assert np.isclose(df_no_grouping.sum().sum() - TEST_DF.sum().sum(), 0.)

    # Check sliced sums
    assert all(df_no_grouping.sum(axis=1) != TEST_DF.sum(axis=1))
    assert all(df_no_grouping[[(1, "A"), (2, "A")]] == 0.)  
    ...
    # Check total sum
    assert np.isclose(df_grouping_column.sum().sum() - TEST_DF.sum().sum(), 0.)

    # Check sliced sums
    assert all(df_grouping_column.sum(axis=1) != TEST_DF.sum(axis=1))    
    assert all(df_grouping_column[[(1, "A"), (2, "A")]] == 0.)  
    ...
    # Check total sum
    assert np.isclose(df_grouping_col2.sum().sum() - TEST_DF.sum().sum(), 0.)

    # Check sliced sums
    assert all(df_grouping_col2.sum(axis=1) != TEST_DF.sum(axis=1))   
    assert all(df_grouping_col2[1] == 0.)  
    ...
    # Check total sum
    assert np.isclose(df_grouping_index.sum().sum() - TEST_DF.sum().sum(), 0.)
