...
def test_load_geostrata_with_column_map(geostrata_excel_file, geostrata_sheet_map, geostrata_column_map):
    """Test loading with column name mapping."""
...
def test_load_geostrata_latitude_intervals(geostrata_excel_file, geostrata_sheet_map, geostrata_column_map):
    """Test that latitude intervals are created."""
    result = load_geostrata(geostrata_excel_file, geostrata_sheet_map, geostrata_column_map)
    
    for df in result.values():
        assert "latitude_interval" in df.columns
        assert pd.api.types.is_categorical_dtype(df["latitude_interval"])
