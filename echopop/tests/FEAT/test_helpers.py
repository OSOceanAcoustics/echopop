...
def test_pivot_aged_weight_proportions():
    ...
    # Create DataFrame
    df = pd.DataFrame(
        [[10.5, 15.2, 8.3, 12.7], [11.5, 16.2, 9.3, 13.7]],
        index=index,
        columns=columns
    )
...
def test_pivot_haul_tables():
    ...
    columns = pd.MultiIndex.from_product(
        [
            ["female", "male", "all"], 
            pd.CategoricalIndex(
                pd.IntervalIndex.from_tuples([(2, 4), (4, 6), (6, 8)]), 
                name="length_bin")
        ],
    ...
    # Index
    index = pd.Index(np.array([1., 2., 3.]), name="haul_num")
...
def test_prepare_aged_biomass_dataframes(sample_ci_grid_data):
    ...
    """
    tables = {"all": sample_ci_grid_data.copy(), "male": sample_ci_grid_data.copy(), "female": sample_ci_grid_data.copy()}
...
def test_write_haul_report(tmp_excel):
    ...
    # Get tables
    tables = {
        sex: feat.pivot_haul_tables(df, sex) for sex in ["male", "female", "all"]
    }
