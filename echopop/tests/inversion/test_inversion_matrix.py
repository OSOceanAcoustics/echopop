...
from echopop.inversion.inversion_matrix import (
    echopop_optim_cb, monte_carlo_initialize, objective, perturb_parameters, 
    estimate_population, invert_transect_number_density, 
    invert_intervals_number_density, InversionMatrix
)
...
def test_monte_carlo_basic_functionality(inv_parameters, sample_frequencies, model_settings):
    ...
    sv_measured = pd.Series([-65., -70., -68.])
...
def test_objective_single_value():
    ...
    result = objective(np.array([-65.]), np.array([-67.]))
    assert result == 2.0
...
def test_perturb_parameters_scale_effect(inv_parameters):
    ...
    small_mag = np.mean([abs(p.value - lmfit_params[name].value) 
                         for name, p in pert_small.items()])
    large_mag = np.mean([abs(p.value - lmfit_params[name].value)
                         for name, p in pert_large.items()])
...
def test_estimate_population_basic_structure():
    ...
    inverted_data = pd.DataFrame({
        ('parameters', 120e3): [InvParameters({'length_mean': {'value': 0.03}, 
                                             'g': {'value': 1.02},
    ...
    nasc_data = pd.DataFrame({120e3: [1000.]})
    nasc_data.columns = pd.MultiIndex.from_arrays([[120e3]], names=['frequency'])
    
    result = estimate_population(
        inverted_data, nasc_data, 
        density_sw=1026., reference_frequency=120e3,
        aggregate_method="interval"
    )
    
    expected_cols = ['nasc', 'number_density', 'biomass_density', 
                    'radius_mean', 'body_volume', 'body_density', 'body_weight']
...
def test_estimate_population_body_calculations():
    ...
    inverted_data = pd.DataFrame({
        ('parameters', 120e3): [InvParameters({
...
def test_invert_intervals_number_density():
    ...
    acoustic_data = pd.DataFrame({'thickness_mean': [5., 10., 8.]})
    parameters = pd.DataFrame({'number_density': [100., 150., 120.]})
...
def test_mc_parameters_generation(inv_parameters):
    ...
    mc_params = MCInvParameters(inv_parameters, mc_realizations=5, 
...
def test_inversion_matrix_creation():
    ...
    cols = pd.MultiIndex.from_product(
        [['sv_mean', 'nasc', 'thickness_mean'], frequencies],
        names=[None, 'frequency']
    )
