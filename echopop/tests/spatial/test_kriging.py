...
def test_get_survey_western_extents_westernmost(sample_mesh_coordinates):
    ...
    for transect_num in result['transect_num'].unique():
        original_transect = mesh_with_lat[
            mesh_with_lat['transect_num'] == transect_num
        ]
...
# ----------------------------
def test_adaptive_search_radius_basic(sample_sparse_distance_matrix, sample_mesh_coordinates, sample_kriging_settings):
...
def test_adaptive_search_radius_with_strategy(sample_sparse_distance_matrix, sample_mesh_coordinates, 
                                            sample_kriging_settings, sample_western_extent):
...
def test_adaptive_search_radius_western_boundary(sample_sparse_distance_matrix, sample_mesh_coordinates, 
                                               sample_kriging_settings, sample_western_extent):
...
def test_ordinary_kriging_matrix_symmetry(sample_local_distance_matrix, sample_variogram_parameters):
...
# Test kriging_lambda
# --------------------
def test_kriging_lambda_basic(sample_anisotropy_value, sample_lagged_semivariogram, 
                            sample_kriging_covariance_matrix):
    """Test basic functionality of kriging_lambda."""
    result = kriging_lambda(
        sample_anisotropy_value,
        sample_lagged_semivariogram,
        sample_kriging_covariance_matrix
    )
...
def test_kriging_point_estimator_at_sample_point(sample_biomass_data, sample_variogram_parameters):
    ...
    # Create a simple mesh
    mesh = pd.DataFrame({
        'x': [0.0, 1.0],  # Use points close to sample data
        'y': [0.0, 1.0]
    })
...
def test_project_kriging_results_basic(sample_kriging_results, sample_projection_parameters):
    ...
    transect_df = pd.DataFrame({
        'x': [0.0, 1.0, 2.0],
        'y': [0.0, 1.0, 2.0],
        'biomass_density': [10.0, 15.0, 12.0]
    })
