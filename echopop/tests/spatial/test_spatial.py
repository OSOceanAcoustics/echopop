...
from echopop.nwfsc_feat.spatial import standardize_coordinates, lag_distance_matrix, filter_lag_matrix
...
def test_standardize_coordinates_basic(sample_coordinates_df):
    ...
    assert all(isinstance(x, (int, float, np.number)) for x in result_df["x"]), "x values should be numeric"
    assert all(isinstance(y, (int, float, np.number)) for y in result_df["y"]), "y values should be numeric"
...
def test_filter_lag_matrix_with_azimuth(sample_lag_matrix, sample_mask_matrix, sample_azimuth_matrix):
    ...
    assert len(filtered_matrix) <= np.sum(sample_mask_matrix), "Azimuth filtering should reduce elements"
