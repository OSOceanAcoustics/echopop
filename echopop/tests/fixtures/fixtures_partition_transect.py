...
def age1_nasc_proportions():
    ...
    return pd.Series([0.1, 0.15, 0.2], 
                     index=[1, 2, 3], name='proportion').rename_axis('stratum_ks')


@pytest.fixture
def age1_abundance_proportions():
    """Create age-1 abundance proportions for testing."""
    return pd.Series([0.12, 0.18, 0.22], 
                     index=[1, 2, 3], name='proportion').rename_axis('stratum_ks')
...
def age1_biomass_proportions():
    ...
    return pd.Series([0.08, 0.14, 0.18], 
                     index=[1, 2, 3], name='proportion').rename_axis('stratum_ks')
