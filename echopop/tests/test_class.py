...
def test_VariogramBase_class(description):
    ...
    # -------------------------
    # Evaluate [ DICTIONARY ] AND Assert
    assert set(["model", "n_lags", "lag_resolution", "max_range", "sill", 
                "nugget", "hole_effect_range", "correlation_range", "enhance_semivariance", 
                "decay_power"]).issubset(key_args)
    ...
    with pytest.raises(
        AssertionError, 
        match=re.escape(
            "Values for key 'model' are not the same."
        )
    ):
    ...
    with pytest.raises(
        AssertionError, 
        match=re.escape(
            "Values for key 'n_lags' are not close."
        )
    ):
        assert_dictionary_values_equal(VariogramBase.DEFAULT_VALUES, ORIGINAL_DEFAULTS)
    # ---- Create typed dictionary
    ...
    assert {params[key] != ORIGINAL_DEFAULTS[key] and params[key] == MOCK_NEW_DEFAULTS[key] 
            for key in MOCK_NEW_DEFAULTS.keys()}
    # ---- Assert equality of input parameters
    assert {params[key] != ORIGINAL_DEFAULTS[key] and params[key] == MOCK_INPUTS[key] 
    ...
    with pytest.raises(
        TypeError, 
        match=re.escape(
            "Value for 'n_lags' (-50, type: 'int') must be a non-negative integer."
        )
    ):
        VariogramBase.update_defaults(MOCK_NEW_DEFAULTS)
...
@pytest.mark.parametrize(
    ...
    [
        (VariogramBase.DEFAULT_VALUES,
         {"model": ["bessel", "exponential"], "n_lags": 30, "lag_resolution": None, 
          "max_range": None, "sill": None, "nugget": None, "hole_effect_range": None, 
          "correlation_range": None, "enhance_semivariance": None, "decay_power": None}, None),
        ({}, 
         VariogramBase.DEFAULT_VALUES, None),
        ({"model": ["bessel", "exponential"], "n_lags": 30, "lag_resolution": None, 
          "max_range": None, "sill": None, "nugget": None, "hole_effect_range": None, 
          "correlation_range": None, "enhance_semivariance": None, "decay_power": None},
         VariogramBase.DEFAULT_VALUES, None),
        ({"sill": 1.00}, 
         {**VariogramBase.DEFAULT_VALUES, **{"sill": 1.00}}, None),
    ...
    ids=[
        "Defaults", "Empty input", "Full input (match defaults)", "Switch 1 input", 
        "Switch 3 inputs", "Invalid `model`", "Invalid `decay_power`",
...
@pytest.mark.parametrize(
    ...
    [
        (VariogramEmpirical.DEFAULT_VALUES,
         {"force_lag_zero": True, "standardize_coordinates": True, "azimuth_range": 360.0}, None),
        ({}, 
        ...
        ({"force_lag_zero": False}, 
         {**VariogramEmpirical.DEFAULT_VALUES, **{"force_lag_zero": False}}, None),
        ({"force_lag_zero": False, "azimuth_range": 180.0}, 
         {**VariogramEmpirical.DEFAULT_VALUES, **{"force_lag_zero": False, "azimuth_range": 180.0}}, 
         None),
    ...
    ids=[
        "Defaults", "Empty input", "Full input (match defaults)", "Switch 1 input", 
        "Switch 2 inputs", "Invalid `force_lag_zero`", "Invalid `azimuth_range`",
        "Input that is same as default"
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        (VariogramOptimize.DEFAULT_VALUES,
         {"max_fun_evaluations": 500, "cost_fun_tolerance": 1e-6, "solution_tolerance": 1e-4, 
          "gradient_tolerance": 1e-4, "finite_step_size": 1e-8, "trust_region_solver": "exact", 
          "x_scale": "jacobian", "jacobian_approx": "forward"}, None),
        ({}, 
         VariogramOptimize.DEFAULT_VALUES, None),
        ({"max_fun_evaluations": 500, "cost_fun_tolerance": 1e-6, "solution_tolerance": 1e-4, 
          "gradient_tolerance": 1e-4, "finite_step_size": 1e-8, "trust_region_solver": "exact", 
          "x_scale": "jacobian", "jacobian_approx": "forward"},
         VariogramOptimize.DEFAULT_VALUES, None),
        ({"max_fun_evaluations": 400}, 
         {**VariogramOptimize.DEFAULT_VALUES, **{"max_fun_evaluations": 400}}, None),
        ({"gradient_tolerance": 1e-6, "trust_region_solver": "base", "x_scale": np.array([1.0])}, 
         {**VariogramOptimize.DEFAULT_VALUES, 
          **{"gradient_tolerance": 1e-6, "trust_region_solver": "base", 
    ...
    ids=[
        "Defaults", "Empty input", "Default input (type: list)", "Switch 1 input", 
        "Switch 3 inputs", "Invalid `trust_region_solver`", "Invalid `finite_step_size`",
        "Input that is same as default"
    ]
)
    ...
    [
        ...
        (VariogramInitial.VALID_PARAMETERS,
         {"correlation_range": {"min": 0.0, "value": 0.0, "max": np.inf}, 
          ...
          "sill": {"min": 0.0, "value": 0.0, "max": np.inf}}, None),
        ...
        ({"sill": {}},
         {"sill": {"min": 0.0, "value": 0.0, "max": np.inf}}, None),
        ({"sill": {"value": 0.5}},
         {"sill": {"min": 0.0, "value": 0.5, "max": np.inf}}, None),
        ({"sill": {"min": 0.1, "value": 0.5}},
         {"sill": {"min": 0.1, "value": 0.5, "max": np.inf}}, None),
        ({"sill": {"min": 0.1, "value": 0.5, "max": 1.0}},
         {"sill": {"min": 0.1, "value": 0.5, "max": 1.0}}, None),
        ({"sill": {"min": 0.1, "value": 0.5, "max": 1.0},
          "nugget": {}, 
          ...
          "correlation_range": {"min": 0.1, "value": 0.2, "max": np.inf}}, None),
        (["invalid_parameter"], 
         None, "Unexpected parameter(s): 'invalid_parameter'"),
        ({"invalid_parameter": {}},
         None, "Unexpected parameter(s): 'invalid_parameter'"),
        (["enhance_semivariance"], 
         None, "Unexpected parameter(s): 'enhance_semivariance'"),
        ({"enhance_semivariance": True}, 
         None, "Value for 'enhance_semivariance' must be a dictionary."),
        ({"enhance_semivariance": {"value": True}}, 
         None, "Unexpected parameter(s): 'enhance_semivariance'"),
        ({"sill": {"min": 0.2}}, 
         None, "Invalid initial values for: 'sill'."),
        ({"nugget": {"min": 0.2, "value": 0.0}}, 
         None, "Invalid initial values for: 'nugget'."),
        ({"decay_power": {"value": 0.2, "max": 0.1}}, 
         None, "Invalid initial values for: 'decay_power'."),
        ({"decay_power": {"value": -0.1, "max": 0.1}}, 
         None, "must be a non-negative real number"),
        ({"decay_power": {"value": 0.0, "max": -0.1}}, 
         None, "must be a non-negative float"),
        ({"decay_power": {"min": -0.2, "value": 0.0}}, 
         None, "must be a non-negative float")
    ],
    ids=[
        "Empty input", "Default output for valid parameters", "User input (type: list)",
        "User input (type: dictionary: empty)", "User input (type: dictionary: value)",
        "User input (type: dictionary: min+value)", "User input (type: dictionary: min+value+max)",
        "User input (type: dictionary: mixed parameters)", "Invalid input (type: list)",
        "Invalid input (type: dictionary)", "Unexpected input (type: list)", 
        "Unexpected input and wrong type (type: dictionary)", 
        "Unexpected input (type: dictionary)", "Invalid case: 'min' > 'value' (no 'value' input)",
        "Invalid case: 'min' > 'value' (provided 'value' input)",
        "Invalid case: 'value' > 'max' (provided 'value' input)",
        "Invalid 'value' datatype", "Invalid 'max' datatype", "Invalid 'min' datatype"
    ]
)
