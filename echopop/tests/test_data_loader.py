...
def test_load_survey_data( mock_survey ,
    ...
    mock_survey.config = load_configuration( Path( test_path[ 'CONFIG' ] / 'config_init.yml' ) , 
    ...
    assert_dictionary_structure_equal( mock_survey.biology , LAYER_NAME_MAP['biological']['data_tree'] )
    assert_dictionary_structure_equal( mock_survey.spatial , LAYER_NAME_MAP['stratification']['data_tree'] )
    assert_dictionary_structure_equal( mock_survey.statistics , LAYER_NAME_MAP['kriging']['data_tree'] )
    ### Data structure
    # ++++ acoustics
    assert mock_survey.acoustics[ 'nasc' ][ 'nasc_df' ].shape == tuple( [ 1 , 10 ] )
    # ++++ biology
    assert mock_survey.biology[ 'catch_df' ].shape == tuple( [ 2 , 7 ] )
    assert mock_survey.biology[ 'distributions' ][ 'age_bins_arr' ].shape == tuple( [ 0 , ] )
    assert mock_survey.biology[ 'distributions' ][ 'length_bins_arr' ].shape == tuple( [ 0 , ] )
    ...
    ### Test merged outputs
    assert set( mock_survey.biology[ 'haul_to_transect_df' ].columns ) <= set( mock_survey.biology[ 'catch_df' ].columns )
    assert set( mock_survey.biology[ 'haul_to_transect_df' ].columns ) <= set( mock_survey.biology[ 'length_df' ].columns )
    assert set( mock_survey.biology[ 'haul_to_transect_df' ].columns ) <= set( mock_survey.biology[ 'specimen_df' ].columns )
    ### Test biological data (sex definition)
    assert np.all( ( mock_survey.biology[ 'length_df' ].sex == 'female' ) & ( mock_survey.biology[ 'length_df' ].group == 'sexed' ) )
    assert np.all( ( mock_survey.biology[ 'specimen_df' ].sex == [ 'male' , 'female' ] ) & ( mock_survey.biology[ 'specimen_df' ].group == 'sexed' ) )
...
def test_biometric_distributions( mock_survey ,
    ...
    # -----------------
    ### Data structure
    assert mock_survey.biology[ 'distributions' ][ 'age' ][ 'age_interval_arr' ].shape == tuple( [ 23 , ] )
    assert mock_survey.biology[ 'distributions' ][ 'age' ][ 'age_bins_arr' ].shape == tuple( [ 22 , ] )
    assert mock_survey.biology[ 'distributions' ][ 'length' ][ 'length_interval_arr' ].shape == tuple( [ 41 , ] )
    assert mock_survey.biology[ 'distributions' ][ 'length' ][ 'length_bins_arr' ].shape == tuple( [ 40 , ] )
    ### Data equality
    assert np.all( mock_survey.biology[ 'distributions' ][ 'age' ][ 'age_interval_arr' ] == np.linspace( 0.5 , 22.5 , 23 ) )
    assert np.all( mock_survey.biology[ 'distributions' ][ 'age' ][ 'age_bins_arr' ] == np.linspace( 1 , 22 , 22 ) )
    assert np.all( mock_survey.biology[ 'distributions' ][ 'length' ][ 'length_interval_arr' ] == np.linspace( 1 , 81 , 41 ) )
    assert np.all( mock_survey.biology[ 'distributions' ][ 'length' ][ 'length_bins_arr' ] == np.linspace( 2 , 80 , 40 ) )