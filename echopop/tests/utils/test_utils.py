...
def test_apply_filters_include():
    ...
    df = pd.DataFrame({
        'species': ['cod', 'hake', 'pollock', 'cod', 'hake'],
...
def test_apply_filters_combined():
    ...
    result = utils.apply_filters(
        df, 
        include_filter={'species': ['cod', 'hake']},
        exclude_filter={'length': 30}
    )
    assert len(result) == 3
    assert all(result['species'].isin(['cod', 'hake']))
...
def test_create_grouped_series(proportion_dict):
    ...
    female_aged_strat1 = result[(result['group'] == 'aged') & 
                               (result['sex'] == 'female') &
                               (result['stratum_num'] == 1)]['proportion_overall'].iloc[0]
