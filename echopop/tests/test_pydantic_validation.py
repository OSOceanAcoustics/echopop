...
@pytest.mark.parametrize(
    ...
    [
        (
            dict(),
            None,
            "Both 'correlation_range' and 'search_radius' arguments are missing"
        ),
        (
        ...
        (
            ...
            ["Input should be greater than or equal to 3", 
             "Input should be greater than or equal to 3",
             "Value must be a non-negative float", "Value must be a non-negative float"]
        ),
        (
            dict(anisotropy = np.nan, kmin=np.nan, kmax=np.nan, correlation_range=np.nan, 
                 search_radius=np.nan),
            None,
            ["Input should be a finite number", "Value must be a non-negative integer", 
             "Value must be a non-negative integer", "Input should be greater than 0",
             "Input should be greater than 0"]
        ),
        (
            dict(anisotropy = np.inf, kmin=np.inf, kmax=np.inf, correlation_range=np.inf, 
                 search_radius=np.inf),
            None,
            ["Value must be a non-negative real number", "Value must be a non-negative integer", 
             "Value must be a non-negative integer", "Value must be a non-negative real number",
...
@pytest.mark.parametrize(
    ...
    [
        (
            dict(),
            dict(best_fit_variogram=False, coordinate_transform=True, extrapolate=False, 
                 variable="biomass", verbose=True),
            None
        ),
        (
        ...
        (
            dict(variable="krakens"),
            None,
            "Input should be 'biomass'"
        ),
    ],
    ids=[
...
@pytest.mark.parametrize(
    ...
    [
        (
            dict(),
            dict(crop_method="transect_ends", num_nearest_transects=4, mesh_buffer_distance=1.25,
                latitude_resolution=1.25, bearing_tolerance=15.0),
            None
        ),
        (
            dict(crop_method="invalid"),
            None,
            "Input should be 'transect_ends' or 'convex_hull'"
        ),
        (
            dict(mesh_buffer_distance=1.0, latitude_resolution=1.0, bearing_tolerance=15),
            dict(crop_method="transect_ends", num_nearest_transects=4, mesh_buffer_distance=1.0, 
                 latitude_resolution=1.0, bearing_tolerance=15.0),
            None
        ),
        (
        ...
        (
            dict(num_nearest_transects=np.nan, mesh_buffer_distance=np.nan, 
                 latitude_resolution=np.nan, bearing_tolerance=np.nan),
            None,
            ["Value must be a non-negative integer.", "Input should be greater than 0", 
             "Input should be greater than 0", "Input should be greater than 0"]
        ),
        (
            dict(num_nearest_transects=np.inf, mesh_buffer_distance=np.inf, 
                 latitude_resolution=np.inf, bearing_tolerance=np.inf),
            None,
            ["Value must be a non-negative integer.", "Value must be a non-negative real number", 
             "Value must be a non-negative real number", "Value must be a non-negative real angle"]
        ),
        (
            dict(bearing_tolerance=181.0),
            None,
