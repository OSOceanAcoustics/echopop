...
def test_impute_missing_sigma_bs( ):
    ...
    mock_sigma_bs_full = pd.DataFrame( { 'stratum_num': [ 1 , 2 , 3 , 4 , 5 ] ,
    ...
    mock_sigma_bs_mid = pd.DataFrame( { 'stratum_num': [ 1 , 2 , 4 , 5 ] ,
    ...
    mock_sigma_bs_edge = pd.DataFrame( { 'stratum_num': [ 2 , 3 , 4 ] ,
                                         'species_id': np.repeat( 94832 , 3 ) ,
                                         'sigma_bs_mean': [ 1e-2 , 1e-3 , 1e-4 ] } )
    # ---- Strata definition dictionary
    mock_strata_dict = {
        'strata_df': pd.DataFrame( { 'stratum_num': [ 1 , 2 , 3 , 4 , 5 ] } )
    }

    # -------------------------
    # Evaluate [ FULL ] 
    test_results_full = impute_missing_sigma_bs( mock_strata_dict ,
                                                 mock_sigma_bs_full )
    # Evaluate [ MID ] 
    test_results_mid = impute_missing_sigma_bs( mock_strata_dict ,
                                                mock_sigma_bs_mid  )
    # Evaluate [ EDGE ] 
    test_results_edge = impute_missing_sigma_bs( mock_strata_dict ,
                                                 mock_sigma_bs_edge )
    ...
    # Expected [ MID ]
    expected_mid = pd.DataFrame( { 'stratum_num': [ 1 , 2 , 3 , 4 , 5 ] ,
                                   'species_id': np.repeat( 94832 , 5 ) ,
                                   'sigma_bs_mean': [ 1e-1 , 1e-2 , 5.05e-3 , 1e-4 , 1e-5 ] } )
    # Expected [ EDGE ]
    expected_edge = pd.DataFrame( { 'stratum_num': [ 1 , 2 , 3 , 4 , 5 ] ,
                                    'species_id': np.repeat( 94832 , 5 ) ,
