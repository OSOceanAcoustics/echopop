...
from echopop.computation.kriging_methods import kriging_lambda , kriging_matrix
...
def test_kriging_lambda( ):
    ...
    # Mock kriging matrix
    test_kriging_matrix = np.array( [ [ 0.00 , 0.11 , 0.20 , 0.24 , 0.25 , 1.00 ] ,
    ...
    eval_kriging_lambda = kriging_lambda( test_anisotropy , 
                                          test_lagged_semivariogram , 
                                          test_kriging_matrix )
    ...
    # -----------------
    # Expected outcome
    expected_array = np.array( [ 3.2621651 , -1.01812305 , -0.14593900 ,
                                 -0.16842517 , -0.9296778 , 0.41402284 ] )
    # Test
...
def test_kriging_matrix( ) :
    ...
    eval_kriging_matrix = kriging_matrix( test_x_coordinates , 
                                          test_y_coordinates , 
                                          test_variogram_parameters )
    ...
    # -----------------
    # Expected outcome
    expected_matrix = np.array( [ [ 0.00000000 , 4.14643112 , 4.85428403 , 4.97512428 , 4.99575337 , 1.00000000 ] ,
