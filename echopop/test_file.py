...
survey.kriging_analysis(
    variogram_parameters=dict(model=["exponential", "bessel"], n_lags=30),
...
full_df = pd.concat(
    [
        aggregate_lengths.loc[aggregate_lengths.group_sex == "sexed", :],
...
if sex == "all":
    total_row + [
        "",
        "Male+Female",
        full_pvt.loc["female"].sum().sum() + full_pvt.loc["male"].sum().sum(),
    ]
# ---- Append
ws.append(total_row)
...
class FEATReports:
    ...
    def generate(self):
        ...
        report_files = [v.get("function")(self, **v) for k, v in report_methods.items()]
    ...
    __METHOD_REFERENCE__ = {
        "kriged_aged_biomass_mesh": {
            ...
            "title": None,
            "filename": lambda v: Path(v.save_directory) / "kriged_aged_biomass_dataframe.xlsx",
        },
        "kriged_biomass_mesh": {
