...
def number_proportions_xr(
    ...
    # Validate that the variables are 'count'.
    if isinstance(data, xr.DataArray):
        if data.name != "count" and not (
            "variable" in data.coords and np.all(data.coords["variable"] == "count")
        ):
...
def stratum_averaged_weight_xr(
    ...
    Examples
    --------
    >>> # Prepare xarray Datasets for proportions (see proportions.number_proportions_xr)
    ...
    # ---- These are grouped based on the data dimensions
    grps = list(set(length_weight_data.dims) - set(["length_bin"]))

    # Create aggregate table with summed proportions per group (e.g. strata)
    aggregate_proportions = utils.create_grouped_table(
        number_proportions_cnv,
        group_cols=grps + group_columns,
        index_cols=["group"],
    ...
    length_proportions_group = utils.create_pivot_table(
        length_proportions_df,
        index_cols=["group"] + grps + ["length_bin"],
        strat_cols=group_columns,
        value_col="within_group_proportion",
    )
    # ---- Create table for the combined within-grouped proportions needed for later calculations
    ...
    # Compute the re-weighted proportions from the mixture of the different groups
    adjusted_proportions = calculate_adjusted_proportions(
        proportion_vars, aggregate_proportions, group_proportions, grps
    )
...
def scale_weights_by_stratum_xr(
    ...
    defined by group_columns, the sum of weights matches the total catch weight for that group,
    while preserving the relative proportions within each group.
    ...
    group_columns : list of str, default []
    ...
    Examples
    --------
    >>> standardized_weights = scale_weights_by_stratum_xr(
...
def weight_proportions_xr(
    ...
    Examples
    --------
    >>> weight_props = weight_proportions_xr(
...
def scale_weight_proportions_xr(
    ...
    Examples
    --------
    >>> props = scale_weight_proportions_xr(
    ...
    # Calculate grouped catch data
    grouped_catch_data = catch_data.groupby(group_columns)["weight"].sum().to_xarray()

    # Calculate the total grouped weights
    group_weights = scaled_weight_data.sum(
    ...
    weight_props = scaled_weight_data / total_weights
    ...
    length_weight_props = calculate_within_group_proportions(
        number_proportions_cnv, list(weight_grouped_props.dims)
    )

    # Reindex the number proportions to generate pivot table
    length_weight_props_all = utils.create_pivot_table(
        length_weight_props,
        index_cols=["group", "length_bin"],
        strat_cols=group_columns,
        value_col="proportion",
    ...
    # Calculate the average weights per length bin within each stratum
    fitted_weights = length_weight_props_all.T * binned_weights.to_numpy().squeeze()
...
def get_number_proportions_slice(
    ...
    include_filter : Dict[str, Any], optional
        Dictionary specifying values to include for filtering (e.g., {"age_bin": [1]}).
    ...
    -------
    xr.DataArray
        DataArray containing the calculated NASC proportions for each group/stratum.
...
def get_weight_proportions_slice_xr(
    ...
    Examples
    --------
    >>> weight_props = get_weight_proportions_slice_xr(
    ...     weight_proportions,
    ...     group_columns=["stratum_ks"],
    ...     include_filter={"age_bin": [1]},
    ...     number_proportions={"aged": ds_aged, "unaged": ds_unaged},
    ...
    if len(number_proportions) > 0:
        ...
        if length_threshold_min:
            ...
            filtered_number_proportions_grp = {
                key: get_number_proportions_slice_xr(
                    ds,
