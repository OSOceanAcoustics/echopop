...
VALS = inv_parameters.values
OTHER = model_InversionMatrix_settings
ENVIRONMENT = {
    "sound_speed_sw": 1500.0,
    "density_sw": 1026.9,
}
taper_order = OTHER["taper_order"]
length_sd_norm = VALS["length_sd_norm"]
length_mean = VALS["length_mean"]
length_radius_ratio = VALS["length_radius_ratio"]
radius_of_curvature_ratio = VALS["radius_of_curvature_ratio"]
theta_mean = VALS["theta_mean"]
theta_sd = VALS["theta_sd"]
orientation_distribution = {"family": "gaussian", "bins": 60}
length_distribution = {"family": "gaussian", "bins": 100}
g = VALS["g"]
h = VALS["h"]
sound_speed_sw = ENVIRONMENT["sound_speed_sw"]
n_integration = OTHER["n_integration"]
n_wavelength = OTHER["n_wavelength"]
number_density = VALS["number_density"]
frequency_interval = OTHER["frequency_interval"]
center_frequencies = sample_frequencies
...
from echopop.inversion import utils as ops
from echopop.inversion.pcdwba import fast_phase, batch_interp
...
len(f_bs) == len(sample_frequencies)
len(f_bs[0]) == np.array([theta_mean]).shape[0]
len(f_bs[0][0]) == len(frequencies[0][~np.isnan(frequencies[0])])
f_bs[0][0][0].shape 

f_bs[0][0][5].shape 

len(f_bs_orientation)
f_bs_orientation[0].shape
F_BS_RE = [np.sqrt((mm[0].real ** 2 + mm[0].imag ** 2)) for mm in [f_bs[0]]]
ops.orientation_average(
    theta_values, [f_bs[0]], theta_mean, theta_sd, orientation_distribution["family"]#, output_type="f_bs"
)
