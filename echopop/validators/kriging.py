...
class KrigingParameters(
    ...
    aspect_ratio: float = Field(default=1e-3, gt=0., le=1., allow_inf_nan=False)
    ...
    def validate_k_interval(cls, values):
        # Get `k_min` and `k_max`
        k_min = getattr(values, "k_min", 5); k_max = getattr(values, "k_max", 20)
...
class VariogramKrigeModelParameters(base.BaseDictionary, 
...
class ValidateKrigingClass(base.BaseDictionary, 
                      arbitrary_types_allowed=True, 
                      title="kriging analysis parameters"):
    mesh: pd.DataFrame
...
class ValidateMeshCropArgs(base.BaseDictionary,
               arbitrary_types_allowed=True,
               title="mesh cropping parameters for kriging"):
