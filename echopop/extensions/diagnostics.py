...
class DiagnosticPlot:
    ...
    def mesh_cropping_results(self) -> pn.Column:
        ...
        DATAINPUT = {
            "Latitude/longitude": {
                "cropped": self.data.analysis["kriging"]["mesh_df"].copy() 
                    if "kriging" in self.data.analysis else {},
        ...
        [v.rename(columns={col: "longitude" if "longitude" in col
                           else "latitude" if "latitude" in col
                           else col for col in v.columns},
                  inplace=True) for k, v in DATAINPUT["Latitude/longitude"].items()]

        # Initialize widget options
        # ---- Variable options
        var_options = ["Latitude/longitude", "Isobath-referenced"]
        # ---- Get column names
        var_columns = {"Latitude/longitude": ["longitude", "latitude"], 
        ...
        def update_plot(selection):
            ...
            # ---- Full mesh
            if "full" in data:
                plot_layers.append(gv.Points(data["full"],
                                            kdims=coord,
                                            label="Full mesh").opts(
                                                size=8,
                                                marker="s",
                                                color="gray"
                                                ))
            # ---- Cropped mesh
            plot_layers.append(gv.Points(data["cropped"], 
                                        kdims=coord, 
                                        label="Cropped mesh").opts(
                                            size=8, 
                                            marker="s",
                                            color="black"
                                            ))
            ...
            if selection == "Latitude/longitude": 
                plot_layers.append(gv.Path(isobath_df, 
                                        kdims=["longitude", "latitude"], 
                                        label="200 m isobath").opts(
                                            line_dash="dashed", 
                                            line_width=2, 
                                            color="red", 
                                            alpha=0.75,
            ...
            # ---- Layer the data
            return hv.Overlay(plot_layers).opts(height=800, width=800, 
    ...
    def mesh_regions(self) -> hv.core.overlay.Overlay:
        ...
        HOVERPATH = gv.Points(DATAINPUT,
                              kdims=["longitude", "latitude"], 
                              vdims=["transect_num"]).opts(
                                  alpha=0.0,
                                  tools=[hover_tool]
                                  )
    ...
    def nasc_map(self) -> hv.core.overlay.Overlay:
        ...
        gdf = gpd.GeoDataFrame(DATAINPUT, 
                               geometry=gpd.points_from_xy(DATAINPUT.longitude, 
                                                           DATAINPUT.latitude))

        # Create `shapely` LineString objects
        lines_gdf = gdf.groupby("transect_num").apply(
        ...
        DATACOPY["nasc_scale"] = (
            (DATACOPY.nasc - DATACOPY.nasc.min()) 
        ...
        # Create point layer
        points_gv = gv.Points(DATACOPY,
                              kdims=["longitude", "latitude"],
                              vdims=["transect_num", "nasc", "nasc_scale", "nasc_size"]).opts(
                                  # ---- COLOR
                                  colorbar=True,
        ...
        full_plot = (transect_gv * self.tile * points_gv).opts(height=800,
                                                               width=800,
                                                               xlabel="Longitude (\u00B0E)",
                                                               ylabel="Latitude (\u00B0N)")
        # ---- Return plot
    ...
    def stratified_results(self) -> pn.Column:
        ...
        # ---- y-axis names
        y_axis_label = {"Biomass": "Biomass (kmt) [mean ± 95% CI]", 
        ...
        def update_plot(selection):
            ...
            if "kriging" in data:
                # ---- Add errorbar
                kriging_pt_err = hv.Segments(data["kriging"], 
                                            kdims=["stratum", "lower", "stratum", "upper"],
                                            label="Kriging").opts(color="red")            
                # ---- Point estimates
                kriging_pts = hv.Points(data["kriging"], 
                                        kdims=["stratum", value], 
                                        label="Kriging").opts(
                                            size=12, 
                                            color="red",
                ...
                if "transect" in data:
                    # ---- Add errorbar
                    transect_pt_err = hv.Segments(data["transect"], 
                                                  kdims=["stratum", "lower", "stratum", "upper"],
                                                  label="Transect").opts(color="black")    
                    # ---- Point estimates  
                    transect_pts = hv.Points(data["transect"], 
                ...
                # ---- Layer the data
                return hv.Overlay(plot_layers).opts(height=600, width=800, 
                                                    xlabel="INPFC stratum", 
                                                    ylabel=y_axis_label[selection])
