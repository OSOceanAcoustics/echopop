...
def write_haul_report(
    ...
    for sex in ["all", "female", "male"]:
        ...
        # Set up the headers for the file sheet
        ws.append(format_table_headers(sheet_data, 
                                       col_name=f"Haul Number ({sex.capitalize()})"))
    ...
    *generate()*
    ...
    def __init__(self, 
                 ...
                 reports: Optional[List[
                     Literal["aged_length_haul_counts",
                             "kriged_aged_biomass_mesh",
                             "kriged_biomass_mesh",
                             "kriged_length_age_biomass",
                             "kriging_input",
        ...
        # Assign table-type(s)
        if reports is None:
            reports = ["aged_length_haul_counts",
                       "kriged_aged_biomass_mesh",
                       "kriged_biomass_mesh",
                       "kriged_length_age_biomass",
                       "kriging_input",
    ...
    def generate(
        ...
        report_files = [v.get("function")(self, **v) 
                        for _, v in report_methods.items()]
        
        # Map methods to the appropriate report-type
        # ---- Initialize an empty list
        report_list = sum(([item] if isinstance(item, str) else list(item) 
    ...
    def aged_length_haul_counts_report(
        ...
        **kwargs,    
        ...
        # Initial pivot table
        full_pvt = full_data.pivot_table(index=["sex", "length_bin"], 
                                         columns=["haul_num"], 
                                         values="length", 
                                         aggfunc="count", 
                                         observed=False)
        ...
        haul_pvt_tables = {sex: pivot_haul_tables(full_pvt, sex) 
                            for sex in ["all", "female", "male"]}
    ...
    def kriged_aged_biomass_mesh_report(
        ...
        **kwargs,
        ...
        mesh_pvt_reduced_tables = {sex: mesh_pvt_tables[sex].loc[lambda x: x.biomass > 0.0]
                                   for sex in ["all", "female", "male"]}
    ...
    def kriging_input_report(
        ...
        **kwargs,
        ...
        dataset_filt = dataset.filter(["latitude", "longitude", "biomass_density", "nasc", 
                                       "number_density"])
    ...
    def total_length_haul_counts_report(
        ...
        **kwargs,
        ...
        full_length_data = pd.concat(
            [length_data.loc[length_data.group_sex == "sexed", :], 
        ...
        # ---- Specimen
        full_specimen_data = full_specimen_data.loc[(~np.isnan(full_specimen_data.length)) 
                                                    & (~np.isnan(full_specimen_data.age))]

        # Initial pivot table
        # ---- Length
        full_length_pvt = full_length_data.pivot_table(index=["sex", "length_bin"], 
                                                       columns=["haul_num"], 
                                                       values="length_count", 
                                                       aggfunc="sum", 
                                                       observed=False)
        # ---- Specimen
        full_specimen_pvt = full_specimen_data.pivot_table(index=["sex", "length_bin"], 
                                                           columns=["haul_num"], 
                                                           values="length", 
                                                           aggfunc="count", 
                                                           observed=False)
    ...
    def transect_aged_biomass_report(
        ...
        **kwargs,
        ...
        # Reduce the datasets
        transect_pvt_reduced_tables = {sex: 
            pivot_dataframe_data(transect_df.loc[transect_df.biomass > 0.0], 
                                 tables, 
                                 sex, 
                                 stratum_name) 
            for sex in ["all", "female", "male"]}
    ...
    __METHOD_REFERENCE__ = {
        "aged_length_haul_counts": {
            ...
            "filename": lambda v: 
                Path(v.save_directory) / "aged_length_haul_counts_table.xlsx",
        },
        "kriged_aged_biomass_mesh": {
        ...
        "kriging_input": {
            ...
            "title": None,
            "filename": lambda v:
                Path(v.save_directory) / "kriging_input_dataframe.xlsx",
        },
        "total_length_haul_counts": {
            ...
            "filename": lambda v:
                Path(v.save_directory) / "total_length_haul_counts_table.xlsx",
        },
        "transect_aged_biomass": {
