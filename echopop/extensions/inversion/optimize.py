...
from .scatterer import compute_ts, compute_Sv
import numpy as np
...
def normalize_parameters(scattering_parameters: Dict[str, Any],
    ...
    if inverse is False:
        scattering_parameters_norm = {
            key: {
                **value,
                "initial": (value["initial"] - value["low"]) / (value["high"] - value["low"])
                if value["high"] != value["low"] else 0.0,
            }
            if isinstance(value, dict) else value  # Skip scalar entries
    ...
    # Min-max inverse normalization
        ...
        scattering_parameters_norm = {
            key: {
                **value,
                "initial": value["initial"] * (value["high"] - value["low"]) + value["low"]
                if value["high"] != value["low"] else value["low"],  # Handle edge cases
            }
            if isinstance(value, dict) else value  # Skip scalar entries
            for key, value in scattering_parameters.items()
        }
...
def simulate_Sv(
    ...
    frequencies = generate_frequency_interval(processing_parameters["center_frequencies"], 
                                              length_sd_norm, 
                                              processing_parameters["frequency_interval"])

    # Compute the acoustic wavenumbers weighted by target size
    # ---- Center frequencies
    k_center = wavenumber(processing_parameters["center_frequencies"], 
                           processing_parameters["water_sound_speed"])
    ...
    theta_values = np.linspace(parameters_dict["theta_mean"] - 3.1 * parameters_dict["theta_sd"], 
...
def prepare_optimization(
    ...
    _ = {parameters.add(param, **{key_mapping[k]: v for k, v in attrs.items() if k in key_mapping})
        for param, attrs in scattering_parameters.items() if isinstance(attrs, dict)}
