...
from ..spatial.transect import transect_extent , transect_bearing
...
def crop_mesh( transect_data: pd.DataFrame ,
    ...
    if settings_dict[ 'crop_method' ] == 'interpolation':
        return ( 
            interpolate_crop_method( transect_data.copy( ) ,
                                     mesh ,
                                     settings_dict )
        )
    # ---- Convex hull
        ...
        return (
            hull_crop_method( transect_data.copy( ) ,
                              mesh ,
                              settings_dict )
        )

def hull_crop_method( transect_data: pd.DataFrame ,
                      mesh_data: pd.DataFrame ,
                      settings_dict: dict ) :
    ...
    mesh_gdf = gpd.GeoDataFrame( mesh_data ,
                                 geometry = gpd.points_from_xy( mesh_data[ 'longitude' ] ,
                                                                mesh_data[ 'latitude' ] ) ,
...
def interpolate_crop_method( transect_data: pd.DataFrame ,
                             mesh_data: pd.DataFrame ,
                             settings_dict: dict ) :
    ...
    # ---- Mean latitude
    transect_data[ 'latitude_mean' ] = (
        transect_data.groupby( [ 'transect_num' , 'mesh_region' ] )[ 'latitude' ]
        .transform( 'mean' )
    )
    # ---- Northernmost extent
    transect_data[ 'latitude_north' ] = (
        transect_data.groupby( [ 'transect_num' , 'mesh_region' ] )[ 'latitude' ]
        .transform( 'max' )
    )
    # ---- Southernmost extent
    transect_data[ 'latitude_south' ] = (
        transect_data.groupby( [ 'transect_num' , 'mesh_region' ] )[ 'latitude' ]
        .transform( 'min' )
    )
    # ---- Eastern extent
    transect_data[ 'longitude_east' ] = (
        transect_data.groupby( [ 'transect_num' , 'mesh_region' ] )[ 'longitude' ]
        .transform( 'max' )
    )
    # ---- Westernmost extent
    transect_data[ 'longitude_west' ] = (
        transect_data.groupby( [ 'transect_num' , 'mesh_region' ] )[ 'longitude' ]
        .transform( 'min' )
    )
    # ---- Index by region
    ...
    # ---- Region 1
    region_1_latitude = (
        np.arange( transect_data.loc[ 1 , 'latitude' ].min( ) ,
    ...
    # -------- Compute the array
    region_2_longitude = (
        np.arange( transect_data.loc[ 2 , 'longitude_west' ].min( ) ,
                   transect_data.loc[ 2 , 'longitude_east' ].max( ) ,
                   longitude_resolution_deg )
    )
    # ---- Region 3
    region_3_latitude = (
        np.arange( transect_data.loc[ 3 , 'latitude_south' ].min( ) ,
    ...
    transect_north = (
        transect_data
        .loc[ transect_data[ 'latitude' ] == transect_data[ 'latitude_north' ] ].loc[2]
    )
    # -------- South
    transect_south = (
        transect_data
        .loc[ transect_data[ 'latitude' ] == transect_data[ 'latitude_south' ] ].loc[2]
    )
    # -------- Iterate through
    ...
    transect_west = (
        transect_data
        .loc[ transect_data[ 'longitude' ] == transect_data[ 'longitude_west' ] ].loc[3]
    )
    # -------- East
    transect_east = (
        transect_data
        .loc[ transect_data[ 'longitude' ] == transect_data[ 'longitude_east' ] ].loc[3]
    )
    # -------- Iterate through
...
def mesh_to_transects( kriging_dict: dict ,
                       spatial_dict: dict ,
    ...
    latitude_bins = latitude_bins = np.concatenate( [ [ -90.0 ] , 
                                                     strata[ 'northlimit_latitude' ] , 
                                                     [ 90.0 ] ] )
    ...
    mesh_data.loc[ : , f"{stratum_col}"] = pd.cut( mesh_data[ 'latitude' ] ,
                                                  latitude_bins ,
    ...
    virtual_transect_data = (
        mesh_data.reset_index( )[ [ 'transect_num' , 'longitude' , 'latitude' , 
                                    stratum_col , variable_col  ] ]
    )
    ...
    virtual_transect_summary[ 'longitude_min' ] = (
        virtual_transect_data.groupby( [ 'transect_num' ] )[ 'longitude' ].min( )
    )
    # ---- Calculate the mean longitude
    virtual_transect_summary[ 'longitude_mean' ] = (
        virtual_transect_data.groupby( [ 'transect_num' ] )[ 'longitude' ].mean( )
    )    
    # ---- Calculate the minimum longitude
    virtual_transect_summary[ 'longitude_min' ] = (
        virtual_transect_data.groupby( [ 'transect_num' ] )[ 'longitude' ].min( )
    ...
    virtual_transect_summary[ 'longitude_max' ] = (
        virtual_transect_data.groupby( [ 'transect_num' ] )[ 'longitude' ].max( )
    )
    # ---- Calculate virtual transect distances
    virtual_transect_summary[ 'transect_distance' ] = virtual_transect_summary.apply( lambda row:
    ...
    virtual_transect_summary[ 'mean_spacing' ] = virtual_transect_summary.apply( lambda row:
        geopy.distance.distance(
    ...
    virtual_transect_summary[ 'transect_area' ] = np.where(
        ~ virtual_transect_summary.index.isin( [ 0 , virtual_transect_summary.index[ -1 ] ] ) ,
        virtual_transect_summary[ 'transect_distance' ] * virtual_transect_summary[ 'mean_spacing' ] / 2.0 ,
    ...
    virtual_strata_summary[ 'transect_count' ] = (
        virtual_transect_summary.groupby( [ stratum_col] , observed = False ).size()
    )
    # ---- Calculate the total area
    virtual_strata_summary[ 'transect_area_total' ] = (
        virtual_transect_summary.groupby( [ stratum_col] , observed = False )[ 'transect_area' ].sum( )
    )
