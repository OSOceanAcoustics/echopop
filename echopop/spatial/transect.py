...
def correct_transect_intervals( transect_data: pd.DataFrame ,
    ...
    transect_data_copy[ 'interval' ] = (
       np.where( np.abs( transect_data_copy[ 'interval' ] - median_interval ) > interval_threshold ,
    ...
    return (
        transect_data_copy
        .filter( regex = '^(?=transect_num|latitude|longitude|stratum_|haul_|interval_area|NASC_).*' )
    )
...
def save_transect_coordinates( transect_data: pd.DataFrame ) :
    ...
    return(
        transect_data[ [ 'transect_num' , 'stratum_num' , 'stratum_inpfc' , 'longitude' , 'latitude' ,
                         'transect_spacing' ] ]
    )
...
def prepare_transect_strata( transect_dict: dict , 
    ...
    transect_info[ f"{settings_dict[ 'variable' ]}" ] = (
       transect_data[ f"{settings_dict[ 'variable' ]}" ]
    )
...
def transect_distance( transect_data: pd.DataFrame ):
    ...
    transect_spatial = (
        transect_data
        .drop_duplicates( [ stratum_col , 'transect_num' ] ).set_index( 'transect_num' ) 
    )
    # ---- Mean latitude
    transect_spatial[ 'latitude_mean' ] = (
        transect_data.groupby( [ 'transect_num' ] )[ 'latitude' ].mean( )
    ) 
    # ---- Min longitude
    transect_spatial[ 'longitude_min' ] = (
        transect_data.groupby( [ 'transect_num' ] )[ 'longitude' ].min( )
    )
    # ---- Max longitude
    transect_spatial[ 'longitude_max' ] = (
        transect_data.groupby( [ 'transect_num' ] )[ 'longitude' ].max( )
    )
    # ---- Calculate mean distance (nmi)
    transect_spatial[ 'transect_distance' ] = transect_spatial.apply( lambda row:
        geopy.distance.distance(
           ( row[ 'latitude_mean' ] , row[ 'longitude_min' ] ) ,
           ( row[ 'latitude_mean' ] , row[ 'longitude_max' ] )
        ).nm , axis = 1 )
    # ---- Calculate mean spacing
    transect_spatial[ 'mean_spacing' ] = (
        transect_data.groupby( [ 'transect_num' ] )[ 'transect_spacing' ].mean( )
    )
...
def summarize_transect_strata( transect_summary: pd.DataFrame ) :
    ...
    strata_summary[ 'transect_count' ] = (
        transect_summary.groupby( [ stratum_col ] )[ 'transect_num' ].size( )
    ) 
    # ---- Sum the total transect area within each stratum
    strata_summary[ 'transect_area_total' ] = (
        transect_summary.groupby( [ stratum_col ] )[ 'transect_area' ].sum( )
    )      

    # Return output
    return strata_summary.reset_index( )

def transect_array( index_matrix: np.ndarray , 
...
def transect_extent( transect_data: pd.DataFrame ,
                     projection: str ,
    ...
    transect = transect_data.copy( ).rename( columns = { f"{lon_col}": 'longitude' ,
                                                         f"{lat_col}": 'latitude' } )

    # Convert to GeoDataFrame
    transect_gdf = gpd.GeoDataFrame(
        transect ,
        geometry = gpd.points_from_xy( transect[ 'longitude' ] ,
                                       transect[ 'latitude' ] ) ,
        crs = projection
    )
    ...
    transect_centroid = (
        transect_gdf.groupby( 'transect_num' )[ 'geometry' ].apply( transect_coordinate_centroid )
    ) 
    ...
    for transect in transect_centroid.index :
        ...
        nearest_transects = (
            other_centroids[ other_centroids.distance_centroid.isin( nearest_centroids ) ]
        )
...
def define_western_extent( transect_data: pd.DataFrame , 
    ...
    western_extent_idx = (
        transect_thresholded.groupby( [ 'transect_num' ] )[ [ 'x' ] ].idxmin( )
    )
