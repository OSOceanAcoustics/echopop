function construct_catch_trawl_output_matrices
% construct the hake (or taget species) catch output matrix for visualization 
...
for i=1:length(data.bio.gear.trawl_no)
    %% length-sex samples (station #1)
    n_len_gender=[];
    for j=1:n_hake_len_gender_trawl
        if data.bio.gear.trawl_no(i) == data.bio.hake_length_sex(j).trawl_no
    ...
    for k1=1:n_tot_trawl
        if data.bio.gear.trawl_no(i) == data.bio.catch(k1).trawl_no 
...
%     data.final.table.trawl(1,5)=0;
    if ~isempty(n_len_gender)
        ...
        data.final.table.trawl(indx1,1)=data.bio.gear.trawl_no(i);    % trawl number
        data.final.table.trawl(indx1,2)=data.bio.gear.transect(i);    % transect number
        %% corresponding index in the trawl structure
        trawl_ind=find(data.bio.trawl.trawl_no == data.bio.gear.trawl_no(i));
        ...
        indx_stratum=find(haul_strata(:,1) == data.bio.gear.trawl_no(i));
...
%     end
    ...
    for j=1:n_hake_len_wgt_gender_age_trawl
        if data.bio.gear.trawl_no(i) == data.bio.hake_length_weight_sex_age(j).trawl_no
    ...
    if ~isempty(n_len_wgt_gender_age) & n_len_wgt_gender_age ~= 0
        ...
        data.final.table.trawl(indx2,1)=data.bio.gear.trawl_no(i);    % trawl number
