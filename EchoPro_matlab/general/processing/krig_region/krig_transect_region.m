...
lat_1=lat_1a:dlat:lat_1b; % latitude array for region 1
...
for i=1:length(tx_l)
    ...
    if ~isempty(ind0)
        ...
        if k> 1 & lat_w(k) == lat_w(k-1)
            ...
            %         if i >= 83 & i <= 84
            %             figure(4)
            %             plot(lon_t(ind0),lat_t(ind0),'.-')
            %             axis([-128.5 -126.5 49 50])
            %             grid on
            %             hold on
            %         end
            k=k+1;
...
for i=1:length(tx_r)
    ...
    if ~isempty(ind0)
        ...
        %         if lon_e(k) < -131.2
        %             disp([i tx_r(i)  min(ind0) ind1])
        %         end
        %% overwrite kth transect if it has a duplicated latitude
        if k> 1 & lat_e(k) == lat_e(k-1)
            %               disp([i k floor(tx_l(i)) lat_e(k)])
        else
            k=k+1;
        end
    end
