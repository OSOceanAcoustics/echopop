...
%   [Vp,Ep,eps]=krig2d_old(xg,yg,x,y,var,model_para,hmsg); % checked Vg against Vp, and Eg against Ep
else
   ...
   [vp,ep,ep_s]=krig3d(xp,yp,zp,ep_sx,y,z,var,model_para,hmsg);
	if para.krig.model == 1			% add in the mean for simple kriging
		vp=vp+var_mean;
	end
   Vg=reshape(vp,ny,nx,nz);
   Eg=reshape(ep,ny,nx,nz);
   ep_s=reshape(eps,ny,nx,nz);
 %  [Vp,Ep,Eps]=krig3d_old(xg,yg,zg,x,y,z,var,model_para,hmsg); % checked Vg against Vp, and Eg against Ep
end
...
data.out.krig.err=ep(:);     % kriging variance
