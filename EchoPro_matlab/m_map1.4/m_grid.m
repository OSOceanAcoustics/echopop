...
if ~IsOctave,
   ...
   a=version;
   Matlab_Release_year=ver;
   para.Matlab_Release_year=str2num(Matlab_Release_year(1).Date(8:11));
   if para.Matlab_Release_year < 2015
        m_grid_opt='-bitmax';
   else
        m_grid_opt='-flintmax';
   end
   %if  (sscanf(a(1:3),'%f') >6.0 & sscanf(a(1:3),'%f') <7.4)  & ~ispc,
     patch('xdata',X(:),'ydata',Y(:),'zdata',eval(m_grid_opt)*ones(size(X(:))),'facecolor',gbackcolor,...
