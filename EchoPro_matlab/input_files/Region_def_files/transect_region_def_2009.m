function [tx0,tx1,tx_out1,tx_out2]=transect_region_def_2009(region)

tx0=[];
tx1=[];
tx_out1=[];
tx_out2=[];
if region == 1
%% region 1: paralell transects to latitudes from south of SCB to west of QC IS
    tx0=1;    % southern most transect number
    tx1=112;  % northern most transect number
    %% #.1 = west end of transect
    %% #.4 = east end of transect
    %% left (west) bound
    tx_l=[[tx0:96 98:110]+0.1  113.9];
    %% right (east) bound
    tx_r=[[tx0:104]+0.4 112.9];
    tx_out1=tx_l;
    tx_out2=tx_r;
elseif region == 2
%% region 2: transects paralell to longitudes north of QCI
    tx0=111;  % east most transect number
    tx1=124;  % west most transect number
%% specifies lower (south) and upper (north) region boundaries based on the transects
    %% #.1 = west end of transect
    %% #.4 = east end of transect
    %% #.6 = south end of transect
    %% #.9 = north end of transect
    tx_l=[111.9 109.4 113.6 114.6 115.6 116.6 117.6 127.4 124.1];
    tx_u=[111.9 112.6 [112:117]+0.9 124.4];
    tx_out1=tx_l;
    tx_out2=tx_u;
else
    %% region 3: paralell transects to latitudes west of QC IS
    tx0=96;    % southern most transect number
    tx1=124;    % northern most transect number
    %% specifies left (west) and right (east) region boundaries based on the transects
    %% #.1 = west end of transect
    %% #.4 = east end of transect
    %% #.6 = south end of transect
    %% #.9 = north end of transect
    tx_l=[124 127:134 139 98]+0.1;
    tx_r=[[124 117 128:133]+0.4 96.1];
    tx_out1=tx_l;
    tx_out2=tx_r;
end

return