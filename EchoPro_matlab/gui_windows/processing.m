...
%% re-load the parameter setting file for the selected year
load_proc_parameters(para.survey_year);

switch para.bio_data_type
    case 1
        para.bio.species_code_ID = 22500;   % target species_code for acoustic survey
    case 2
        para.bio.species_code_ID = 22500;   % target species_code for bottom trawl survey
    case 3
        para.bio.species_code_ID = 206;     % target species_code for industry data (observer data)
end
...
% --- Outputs from this function are returned to the command line.
function varargout = adio_default_para_OutputFcn(hObject, eventdata, handles) 
% varargout  cell array for returning output args (see VARARGOUT);
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
para.proc.default_para=get(hObject,'value');
% if para.proc.default_para == 1
%     set(hObject,'value',0)
% elseif para.proc.default_para == 0
%     set(hObject,'value',1)
% end
hdl.proc=handles;
return
