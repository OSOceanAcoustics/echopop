...
ax.scatter(
    nasc_pts,
    np.full_like(nasc_pts, 1.1, dtype=float),
    color=raw_color,
    edgecolor=raw_edge,
    s=90,
    marker="s",
...
ax.scatter(
    on_effort,
    np.full_like(on_effort, 1.0),
    color=on_color,
    edgecolor=on_edge,
    s=90,
    marker="s",
    label="On-effort interval",
    zorder=4,
)
...
for pt in off_effort_pts:
    ...
    ax.scatter(
        ...
        s=90,
        marker="s",
        label="Off-effort interval" if pt == off_effort_pts[0] else None,
        zorder=6,
    )
...
ax.plot(
    on_effort_pts,
    np.full_like(on_effort_pts, 0.8, dtype=float),
...
ax.scatter(
    ...
    np.full_like(on_effort_pts, 0.8, dtype=float),
    color=filtered_color,
    edgecolor=filtered_edge,
    s=90,
    marker="s",
    label="Filtered transect",
    zorder=8,
)
ax.set_ylim(0.7, 1.3)
ax.set_yticks([])
...
plt.tight_layout()
plt.savefig(
    "../../../_static/offeffort_transect_filtering_edges1.png", dpi=150, bbox_inches="tight"
)
...
plt.tight_layout()
plt.savefig(
    "../../../_static/offeffort_transect_filtering_edges2.png", dpi=150, bbox_inches="tight"
)
...
for i, seg in enumerate(on_segments):
    ...
    ax.scatter(
        seg,
        np.full_like(seg, 1.0, dtype=float),
        color=on_color,
        edgecolor=on_edge,
        s=90,
        marker="s",
        label="On-effort interval" if i == 0 else None,
        zorder=4,
    )
...
for i, gap_idx in enumerate(gaps):
    ...
    ax.scatter(
        off_pts,
        np.full_like(off_pts, 0.9, dtype=float),
        color=off_color,
        edgecolor=off_edge,
        s=90,
        marker="s",
        label="Off-effort interval" if i == 0 else None,
        zorder=6,
    )
    # Scatter points for the ends of the interval
    ax.scatter(
        [start, end], [0.9, 0.9], color=off_color, edgecolor=off_edge, s=90, marker="s", zorder=6
    )
...
for i, seg in enumerate(on_segments):
    ax.plot(
        seg,
        np.full_like(seg, 0.8, dtype=float),
        color=filtered_color,
        linewidth=5,
        linestyle="-",
        zorder=7,
    ...
    ax.scatter(
        ...
        s=90,
        marker="s",
        label="Filtered transect" if i == 0 else None,
        zorder=8,
    )
...
plt.tight_layout()
plt.savefig(
    "../../../_static/offeffort_transect_filtering_interior.png", dpi=150, bbox_inches="tight"
)
