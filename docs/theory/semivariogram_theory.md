...
The classic empirical semivariogram assumes homoscedasticity (homogeneous variance) across space. Many real datasets exhibit heteroscedasticity, where local variance changes with  location due to patchiness, measurement error, or sampling design. This can bias or destabilize semivariogram estimates. To reduce sensitivity to local variance differences and outliers, the empirical semivariogram from {ref}`Eq. (3.7) <eq-37>` is normalized by the product of their local standard deviations:
