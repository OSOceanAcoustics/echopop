...
Although these samples may be distant, enforcing the minimum count ensures that the kriging matrix remains well posed and that weights can still be computed reliably. Once $\mathcal{N}_i$ has been determined, the kriging system is assembled in the usual way using only those indices. The weights $\{\lambda_j\}$ and subsequent prediction $\hat{Z}(x_i)$ are identical to the standard formulation used in ordinary kriging.
