...
The underlying random process $Z(\mathbf{x})$ ({ref}`Eq. 3.1 <eq-31>`) can be characterized by both deterministic and stochastic components:
...
A major advantage conferred by using Ordinary Kriging lies in the **unbiasedness constraint**. To ensure the prediction is unbiased regardless of $\mathbf{m}$, the coefficient preceding $\mathbf{m}$ in {ref}`Eq. (3.19) <eq-319>` must be zero. This therefore requires:
...
which results with the because the bracketed term in {ref}`Eq. (3.19) <eq-319>` being zeroed out:
...
Consequently, the unbiasedness constraint eliminates $\mathbf{m}$ from $\mathbf{z}^*(\mathbf{u})$, therefore simplifying {ref}`Eq. (3.19) <eq-319>` to:

<a id="intext_eq_321"></a>
...
The constrained minimization problem, minimizing $\sigma_E^2$ subject to $\sum\lambda_b = 1$, is solved using the **Lagrange Multiplier technique**. This is accomplished by introducing a the Lagrange multiplier, $\mu$, which enforces the constraint during the minimization. The objective function ({ref}`Eq. 3.24a <eq-324a>`) is combined with the constraint using the Lagrangian function:
...
which is $\sum\limits_{j=1}^n \lambda_j = 1$. Using both {ref}`Eq. (3.26b) <eq-326b>` and {ref}`Eq. (3.26c) <eq-326c>` forms the system of equations:

<a id="intext_eq_327"></a>
...
In addition to the kriging variance, it is often useful to report the sample variance or the coefficient of variation $\text{CV}$ for each prediction. The coefficient of variation is defined as
