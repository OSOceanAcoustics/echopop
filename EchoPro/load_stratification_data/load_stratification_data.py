...
class LoadStrataData:  # TODO: Does it make sense for this to be a class?
    ...
    def _load_stratification_file(self):
        ...
        if self.epro.params['strata_sheetname'] in ['Base KS', 'INPFC']:

            # read and check stratification file
            strata_df = pd.read_excel(self.epro.params['data_root_dir'] + self.epro.params['strata_filename'],
                                      sheet_name=self.epro.params['strata_sheetname'])
    ...
    def _load_geographic_stratification(self):
        ...
        if self.epro.params['geo_strata_sheetname'] in ['stratification1', 'INPFC']:

            # read and check geographic stratification file
            geo_strata_df = pd.read_excel(self.epro.params['data_root_dir'] + self.epro.params['geo_strata_filename'],
                                          sheet_name=self.epro.params['geo_strata_sheetname'])
            ...
            self.epro.geo_strata_df = geo_strata_df
    ...
    def _get_strata_sig_b(self):
        ...
        These values are then stored in self.epro.strata_sig_b
        ...
        spec_df = self.epro.specimen_df[['Length', 'Weight']].copy()
