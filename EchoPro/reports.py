...
class Reports:
    ...
    def __process_catch_data_df(df):
        ...
        df = df[
            ["Haul", "Species_Code", "Species_Name", "Number_In_Haul", "Weight_In_Haul"]
        ].copy()
    ...
    def __process_catch_data_ds(df):
        ...
        series_haul_pad = series_haul.apply(
            lambda x: np.vstack((x, np.nan * np.ones((max_haul_size - x.shape[0], 4))))
            if x.shape[0] != max_haul_size
            else x
        )
        ...
        ds = xr.Dataset(
            data_vars={
                "species_code": (
                    ["haul", "haul_index"],
                    np_haul_pad[:, 0].reshape(
                        (series_haul_pad.shape[0], max_haul_size)
                    ),
                ),
                "species_name": (
                    ["haul", "haul_index"],
                    np_haul_pad[:, 1].reshape(
                        (series_haul_pad.shape[0], max_haul_size)
                    ),
                ),
                "number_in_haul": (
                    ["haul", "haul_index"],
                    np_haul_pad[:, 2].reshape(
                        (series_haul_pad.shape[0], max_haul_size)
                    ),
                ),
                "weight_in_haul": (
                    ["haul", "haul_index"],
                    np_haul_pad[:, 3].reshape(
                        (series_haul_pad.shape[0], max_haul_size)
                    ),
                ),
                "no_null_length": (
                    ["haul"],
                    df.index.value_counts().sort_index().to_numpy(),
                ),
            },
            coords={
        ...
        if self.EPro.params["source"] == 3:
            ...
            if self.EPro.params["database_type"] == "Oracle":
                catch_us_df = pd.read_excel(
                    self.EPro.params["data_root_dir"]
                    + self.EPro.params["filename_catch_US"],
                ...
                catch_can_df = pd.read_excel(
                    self.EPro.params["data_root_dir"]
                    + self.EPro.params["filename_catch_CAN"],
                ...
                catch_can_df.index = (
                    catch_can_df.index + self.EPro.params["CAN_haul_offset"]
                )  # add haul_offset
                self.EPro.catch_df = pd.concat([catch_us_df, catch_can_df])

            else:
                raise NotImplementedError(
                    "Loading data from a non-Oracle database has not been implemented!"
                )
        else:
